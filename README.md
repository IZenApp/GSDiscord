...
![GSDiscord Banner](tools/assets/header.svg)
...
- Создать файл `config/.env` с необходимыми переменными
...
5. Поместите файл в `config/credentials/google-credentials.json`
...
4. Добавьте URL в файл `config/.env`
...
![Discord Demo](tools/assets/discord-demo.svg)
...
- 📁 **Изолированное хранение** учетных данных в `config/credentials/`
- 🔑 **Переменные окружения** вместо захардкоженных значений

### Файлы, которые НИКОГДА не попадут в Git:
- `config/.env` (ваши секреты)
- `config/credentials/` (Google Service Account)
...
![Architecture](tools/assets/architecture.svg)
